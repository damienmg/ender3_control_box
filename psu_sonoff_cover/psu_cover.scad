...
module PowerPlug(pos=[0,0,0], rotation=[0,0,0], h=28, w=57, d=10, thickness=2, fillet=1, hole_depth=10, height=15) {
    difference() {
        union() {
        ...
        translate(pos + [height,0,0]) rotate(rotation) {
            translate([-w/2+4, -hole_depth/2, -h/2]) cube([w-8,hole_depth+height,h]);
